...
def process_image(index, fieldsize):
        ...
        for x in range(posx, posx+fieldsize):
            for y in range(posy, posy+fieldsize):
                ...
                pixelacuity[0] = abs(get_bright(image, array[x][y]) - get_bright(image, array[x][y-1]))
                pixelacuity[1] = abs(get_bright(image, array[x][y]) - get_bright(image, array[x+1][y-1]))
                pixelacuity[2] = abs(get_bright(image, array[x][y]) - get_bright(image, array[x+1][y]))
                pixelacuity[3] = abs(get_bright(image, array[x][y]) - get_bright(image, array[x+1][y+1]))
                pixelacuity[4] = abs(get_bright(image, array[x][y]) - get_bright(image, array[x][y+1]))
                pixelacuity[5] = abs(get_bright(image, array[x][y]) - get_bright(image, array[x-1][y+1]))
                pixelacuity[6] = abs(get_bright(image, array[x][y]) - get_bright(image, array[x-1][y]))
                pixelacuity[7] = abs(get_bright(image, array[x][y]) - get_bright(image, array[x-1][y-1]))
                pixelacuity_sum = sum(pixelacuity)
                image_acuties.append(pixelacuity_sum)
        length = len(image_acuties)
        image_acuity = sum(image_acuties)*1.0 / length
...
class Bild(pygame.sprite.Sprite):
    ...
    def load(self, batch):
        ...
        statusString = ''
        self.fieldsize = 500
        self.images_acuity = [0 for x in range(batch)]
        for i in range(80, 100):
            with concurrent.futures.ProcessPoolExecutor(max_workers=1) as executor:
                executor.submit(process_image, i, self.fieldsize)
            #print result
            #self.images_acuity.append(result)
