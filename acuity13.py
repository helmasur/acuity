...
img_dir = "f:/Temp/2/"   # \ does not work as expected
out_dir = data_dir
...
def main():
        ...
        with ProcessPoolExecutor(max_workers=workers) as executor: #this seems to be what actually finishes off processes
            for x in range(workers): #every round this loop creates a set of processes
                if batch_pos >= 0:
                    ...
                    if files[batch_pos] in saved_results:
                        saved_results_list.append((batch_pos, saved_results[files[batch_pos]]))
                        batch_pos -= 1
                    else:
    ...
    for item in results:
        saved_results[files[item[0]]] = item[1]
    ...
    for g in groups:
        ...
        groups_mean.append(int(mean_pos(g, 1)*100)/100.0)
        groups_median.append(int(median_pos(g, 1)*100)/100.0)
        gcopy = g
